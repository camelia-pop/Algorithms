...
We divide the list into two sublists and we assume that each are independetly perfectly shuffled. 
One list has length l and the other has length k. Then a ordering of the elements appear with probability 1/l! and 1/k! in each list.
We merge the two lists by choosing an element from each list with probability proportional to the number of elements remaining in the 
corresponding list. This procedure is implemented in the merge function.
*/
...
/* 
Assume that the sublist that starts from left contains l elements and is shuffled. 
Also assume that the sublist that starts from right contains k elements and is shuffled.
The following procedure merges the two suffled lists into one suffled list of length l + k using the rule: 
the next element to be added to the merged list is chosen with probability p from the first list 
and with probability 1-p from the second list, 
where p is the number of elements that were not already chosen from the first list 
divided by the total number of elements that were not chosen from the two lists.
*/
